...
jasmine.TrivialReporter.prototype.reportSpecResults = function(spec) {
  ...
  for (var i = 0; i < resultItems.length; i++) {
    ...
    if (result.passed && !result.passed()) {
      ...
      var expectationMessage = "Expected " + jasmine.pp(result.actual) + " " + result.matcherName;
      if ("expected" in result) {
        expectationMessage += " " + jasmine.pp(result.expected);
      }
      resultMessageDiv.innerHTML = expectationMessage; // todo: lame; mend
