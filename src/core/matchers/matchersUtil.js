getJasmineRequireObj().matchersUtil = function(j$) {
  ...
  function eq(a, b, aStack, bStack, customTesters, diffBuilder) {
    ...
    if (className == '[object Array]') {
      ...
      var baseIter, baseValue, baseStack;
      var otherSet, otherIter, otherValue, otherStack;
      var found;
      var prevStackSize;
      for (i = 0; result && i < setPairs.length; i++) {
        baseIter = setPairs[i][0];
        otherIter = setPairs[i][1];
        baseStack = stackPairs[i][0];
        otherStack = stackPairs[i][1];
        // For each value in the base set...
        for (var k = 0; result && k < baseIter.length; k++) {
          baseValue = baseIter[k];
          found = false;
          // ... test that it is present in the other set
          for (var l = 0; !found && l < otherIter.length; l++) {
            otherValue = otherIter[l];
